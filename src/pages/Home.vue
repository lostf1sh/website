...
const vscodeStatus = computed(() => {
  ...
  let filename = vscodeActivity.value.details;
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-16">
      <div class="mb-12">
        <div class="mb-8">
          ...
          <div class="text-sm text-catppuccin-gray mb-4">
            <span class="text-catppuccin-subtle">aka</span> <span class="text-catppuccin-green">moli</span>
          </div>
        ...
        <div class="border-l-2 border-catppuccin-surface pl-4 mb-8">
          <div class="text-catppuccin-subtle text-sm mb-3">~$ ps aux | grep duhan</div>
          <div class="space-y-2 text-sm">
            ...
            <div class="flex items-center gap-2">
              ...
              <span class="text-catppuccin-subtle">:</span>
              <span v-if="!isLoading && spotify" class="text-catppuccin-text truncate">
                {{ spotify.song }} - {{ spotify.artist }}
              </span>
              <span v-else class="text-catppuccin-subtle">not playing</span>
            </div>

            <div v-if="!isLoading && vscodeActivity && vscodeStatus && vscodeStatus.details" class="flex items-center gap-2">
              <span class="text-catppuccin-blue">vscode</span>
              <span class="text-catppuccin-subtle">:</span>
              <span class="text-catppuccin-text truncate">
                {{ vscodeStatus.details }}<span v-if="vscodeStatus.state"> in {{ vscodeStatus.state.replace('Workspace: ', '') }}</span>
              </span>
            </div>
          </div>
        </div>
        ...
        <div class="border-l-2 border-catppuccin-surface pl-4 mb-8">
          <div class="text-catppuccin-subtle text-sm mb-3">~$ ls ~/tools</div>
      ...
      <div class="grid lg:grid-cols-2 gap-8">
        <div class="border-l-2 border-catppuccin-surface pl-4">
          ...
          <div v-if="!repos.length" class="text-sm text-catppuccin-subtle">
            no projects found
          </div>
          ...
          <div v-else class="space-y-2">
            <a v-for="repo in repos.slice(0, 6)" :key="repo.id" :href="repo.html_url" target="_blank"
              class="block group">
              <div class="flex items-start gap-3 text-sm hover:text-catppuccin-mauve transition-colors">
                <span class="text-catppuccin-subtle group-hover:text-catppuccin-mauve transition-colors">></span>
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2">
                    <span class="text-catppuccin-text group-hover:text-catppuccin-mauve transition-colors font-medium truncate" :title="repo.name">
                      {{ repo.name }}
                    </span>
                  ...
                  <p class="text-xs text-catppuccin-gray truncate" :title="repo.description">
        ...
        <div class="border-l-2 border-catppuccin-surface pl-4">
          <div class="text-catppuccin-subtle text-sm mb-3">~$ cat recent_tracks.log</div>
          
          <div v-if="songsLoading" class="space-y-2">
            <div v-for="i in 6" :key="`loading-${i}`" class="text-sm text-catppuccin-subtle animate-pulse">
              loading...
            </div>
          </div>
          ...
          <div v-else-if="!consolidatedTracks.length && !currentTrack" class="text-sm text-catppuccin-subtle">
            no tracks found
          </div>

          <div v-else class="space-y-2">
            <a v-if="currentTrack" :href="currentTrack.url" target="_blank" class="block group">
              <div class="flex items-start gap-3 text-sm">
                <span class="text-catppuccin-green">♪</span>
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2">
                    <span class="text-catppuccin-text group-hover:text-catppuccin-green transition-colors truncate" :title="currentTrack.name">
                      {{ currentTrack.name }}
                    </span>
                    <span class="text-catppuccin-green text-xs flex-shrink-0">[now]</span>
                  </div>
                  <p class="text-xs text-catppuccin-gray truncate" :title="currentTrack.artist['#text']">
            ...
            <a v-for="track in consolidatedTracks.slice(0, currentTrack ? 5 : 6)"
              ...
              :href="track.url" target="_blank" class="block group">
              <div class="flex items-start gap-3 text-sm">
                <span class="text-catppuccin-subtle group-hover:text-catppuccin-green transition-colors">></span>
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2">
                    <span class="text-catppuccin-text group-hover:text-catppuccin-green transition-colors truncate" :title="track.name">
                      {{ track.name }}
                    </span>
                    <span v-if="track.playcount > 1" class="text-catppuccin-yellow text-xs flex-shrink-0">
                      ×{{ track.playcount }}
                    </span>
                  </div>
                  <p class="text-xs text-catppuccin-gray truncate" :title="track.artist['#text']">
                    {{ track.artist['#text'] }}
                  </p>
                </div>
              </div>
